...
public class Playback1 : MonoBehaviour 
{
    ...
    void Update()
    {
        if (isLoadingAsync)
        {
            ...
            {
                currentRequest = Resources.LoadAsync<Cubemap>(asyncFiles[asyncIndex]);
                return;
            } else
            {
                if (currentRequest.isDone) // current asset has been loaded
                {
                    sequence[asyncIndex] = currentRequest.asset as Cubemap;
                    currentRequest = null;
                    asyncIndex++;
                }
            }
    ...
    void PlayBack()
    {
        if (isPlaying)
        {
            if (sequence != null)
            {
                if (sequence.Length > 0&&!Input.GetMouseButton(0))
                {
                    //UpdateFrame();
    ...
    void UpdateFrame()
    {
      //  Debug.Log("delta " + delta + " index " + index);
    ...
    {
        Object[] obj = Resources.LoadAll(resourceFolder, typeof(Cubemap));
        if (obj.Length > 0)
        {
            sequence = new Cubemap[obj.Length];
            for (int i = 0; i < obj.Length; i++)
            {
                sequence[i] = (Cubemap)obj[i];
            }
            index = 0;
            UpdateFrame();
    ...
    public void LoadSequence(Cubemap[] textures,bool play = false)
    {
        if (textures != null)
        {
            sequence = textures;
    ...
    public void LoadSequenceAsync(ref List<string> resourcePaths, bool play = false)
    {
        ...
        sequence = new Cubemap[resourcePaths.Count];
    }
