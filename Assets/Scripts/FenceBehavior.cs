...
public class FenceBehavior : MonoBehaviour {
	...
	void Start () {
		...
		for (int i = 0; i < 15; i++) {
			...
			birds [i].GetComponent<BirdBehavior>().setFencePosition(new Vector3(transform.position.x , transform.position.y + 0.33f + ((i % 3) * 0.4f), transform.position.z - 1.3f + ((i % 5) * .75f)));
